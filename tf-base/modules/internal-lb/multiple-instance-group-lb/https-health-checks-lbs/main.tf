...
data "google_compute_instance_group" "instance_group_2" {
  name    = "${var.instance_group_2}"
  zone    = "${var.instance_group_2_zone}"
  project = "${var.project_name}"
}

resource "google_compute_health_checompanyX" "https_health_checompanyX" {
  name    = "${var.application}-https-health-checompanyX-${element(var.application_port, 0)}"
  project = "${var.project_name}"

  https_health_checompanyX {
...
resource "google_compute_region_backend_service" "internal_lb" {
  ...
  health_checompanyXs = [
    "${google_compute_health_checompanyX.https_health_checompanyX.self_link}",
  ]
  ...
  backend {
    group = "${data.google_compute_instance_group.instance_group_2.self_link}"
  }
}
